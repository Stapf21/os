...
class Servicos extends MY_Controller
{
    ...
    public function editar()
    {
        ...
        $servicoId = $this->uri->segment(3);
        $this->data['result'] = $this->servicos_model->getById($servicoId);
        $this->data['itens'] = $this->servicos_model->getItens($servicoId);
    ...
    public function excluir()
    {
        ...
        $this->servicos_model->delete('servicos_itens', 'servicos_id', $id);
    ...
    public function autoCompleteProduto()
    {
        if (isset($_GET['term'])) {
            ...
            $this->servicos_model->autoCompleteProduto($q);
        }
    }
    ...
    public function adicionarItem()
    {
        if (! $this->permission->checkPermission($this->session->userdata('permissao'), 'eServico')) {
            return $this->output
                ...
                ->set_output(json_encode(['result' => false, 'messages' => 'Sem permissao.']));
        }

        $this->load->library('form_validation');
        $this->form_validation->set_rules('idServico', 'Servico', 'trim|required|numeric');
        $this->form_validation->set_rules('idProduto', 'Produto', 'trim|required|numeric');
        $this->form_validation->set_rules('quantidade', 'Quantidade', 'trim|required|numeric|greater_than[0]');
        ...
        if ($this->form_validation->run() === false) {
            ...
            return $this->output
                ...
                ->set_output(json_encode(['result' => false, 'messages' => $errors]));
        }
        ...
        $data = [
            ...
            'editavel' => $this->input->post('editavel') ? 1 : 0,
        ];

        if ($this->servicos_model->add('servicos_itens', $data) == true) {
            log_info('Adicionou item padrao ao servico. ID: ' . $this->input->post('idServico'));
    ...
    public function excluirItem()
    {
        ...
        if (! $id) {
            return $this->output
                ->set_content_type('application/json')
                ->set_status_header(400)
                ->set_output(json_encode(['result' => false, 'messages' => 'Item invalido.']));
        }

        if ($this->servicos_model->delete('servicos_itens', 'idServicoItem', $id) == true) {
            log_info('Removeu item padrao do servico. ID Item: ' . $id);
            return $this->output
                ->set_content_type('application/json')
                ->set_status_header(200)
                ->set_output(json_encode(['result' => true]));
