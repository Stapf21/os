                        ...
                        <div class="tab-pane active" id="tab1">
                            <div class="span12" id="divCadastrarOs">
                                ...
                                <form action="<?php echo current_url(); ?>" method="post" id="formOs">
                                    ...
                                    <div class="span12" style="padding: 1%; margin-left: 0">
                                        <div class="span6">
                                            <label for="servico">Tipo de Servico</label>
                                            <input id="servico" class="span12" type="text" name="servico" value="" />
                                            <input id="idServico" class="span12" type="hidden" name="idServico" value="" />
                                        </div>
                                    </div>
...
<script type="text/javascript">
    $(document).ready(function() {
        function getDescricaoProdutoText() {
            if (typeof tinymce !== 'undefined') {
                var editor = tinymce.get('descricaoProduto');
                if (editor) {
                    return editor.getContent({ format: 'text' }).trim();
                }
            }
            return $.trim($('#descricaoProduto').val());
        }

        function setDescricaoProduto(value) {
            if (typeof tinymce !== 'undefined') {
                var editor = tinymce.get('descricaoProduto');
                if (editor) {
        ...
        $("#servico").autocomplete({
            ...
            select: function(event, ui) {
                ...
                var current = getDescricaoProdutoText();
                if (!current) {
                    setDescricaoProduto(descricao);
                    return;
                }
                if (confirm('Descricao ja preenchida. Deseja substituir pela descricao do servico selecionado?')) {
                    setDescricaoProduto(descricao);
                }
            }
        });
