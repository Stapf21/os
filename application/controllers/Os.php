    ...
    public function adicionar()
    {
        ...
        if ($this->form_validation->run('os') == false) {
            ...
            if (is_numeric($id = $this->os_model->add('os', $data, true))) {
                ...
                $idOs = $id;
                $servicoId = $this->input->post('idServico');
                if (is_numeric($servicoId)) {
                    $this->load->model('servicos_model');
                    $servico = $this->servicos_model->getById($servicoId);
                    if ($servico) {
                        $preco = number_format(getAmount($servico->preco), 2, '.', '');
                        $dataServico = [
                        ...
                        $this->os_model->add('servicos_os', $dataServico);
                    }
                }
