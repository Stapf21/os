...
class Os_model extends CI_Model
{
    ...
    public function getOs($table, $fields, $where = [], $perpage = 0, $start = 0, $one = false, $array = 'array')
    {
        ...
        // remove orcamentos da listagem de OS
        $this->db->where_not_in('os.status', ['Orçamento', 'Orcamento', 'OrÇõamento']);
    ...
    {
        $this->db->select('produtos_os.idProdutos_os, produtos_os.quantidade, produtos_os.preco, produtos_os.os_id, produtos_os.produtos_id, produtos_os.subTotal, produtos.idProdutos, COALESCE(produtos_os.descricao, produtos.descricao) as descricao, produtos.precoVenda');
    ...
    public function autoCompleteServico($q)
    {
        ...
        if ($query->num_rows() > 0) {
            foreach ($query->result_array() as $row) {
                $row_set[] = ['label' => $row['nome'] . ' | Preço: R$ ' . $row['preco'], 'id' => $row['idServicos'], 'preco' => $row['preco'], 'descricao' => $row['descricao']];
            }
            echo json_encode($row_set);
        }
    }

    public function anexar($data)
    {
        return $this->db->insert('anexos', $data);
    }

    public function getAnexo($id)
    {
        return $this->db->where('idAnexos', $id)->get('anexos')->row();
    }

    public function getAnexos($os_id)
    {
        return $this->db->where('os_id', $os_id)->get('anexos')->result();
    }

    public function excluirAnexo($id)
    {
        return $this->db->where('idAnexos', $id)->delete('anexos');
    }
    ...
    public function getUltimaOsPmoc($clientes_id)
    {
        $this->db->from('os');
        $this->db->where('clientes_id', $clientes_id);
        $this->db->where('tipo', 'PMOC'); // ajuste o nome do campo se necessário
        $this->db->order_by('dataFinal', 'desc');
        $this->db->limit(1);
        return $this->db->get()->row();
    }

    public function getHistoricoPmoc($clientes_id)
    {
        $this->db->from('os');
        $this->db->where('clientes_id', $clientes_id);
        $this->db->where('tipo', 'PMOC'); // ajuste o nome do campo se necessário
    ...
    public function vincularEquipamento($os_id, $equipamento_id)
    {
        $data = [
            'os_id' => $os_id,
            'equipamentos_id' => $equipamento_id
        ];
        return $this->db->insert('equipamentos_os', $data);
    }
}
