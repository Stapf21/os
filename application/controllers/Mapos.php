...
class Mapos extends MY_Controller {
    ...
    public function atualizacoes()
    {
        ...
        $status = $this->getDeployStatus($deployConfig);
    ...
    public function executarAtualizacao()
    {
        ...
        if (! $deployConfig['allow_web_trigger']) {
            $this->session->set_flashdata('error', 'Atualizacao via painel esta desativada. Ajuste ALLOW_WEB_DEPLOY no arquivo .env.');
            return redirect(site_url('mapos/atualizacoes'));
        }

        if (stripos(PHP_OS, 'WIN') === 0) {
            $this->session->set_flashdata('error', 'Execucao via painel nao e permitida em ambiente Windows.');
            return redirect(site_url('mapos/atualizacoes'));
        }

        $scriptAbsPath = FCPATH . ltrim($deployConfig['script_path'], '/\\');
        if (! is_file($scriptAbsPath)) {
            $this->session->set_flashdata('error', 'Script de deploy nao encontrado em: ' . $deployConfig['script_path']);
            return redirect(site_url('mapos/atualizacoes'));
        }

        $lockAbsPath = FCPATH . ltrim($deployConfig['lock_file'], '/\\');
        if (is_file($lockAbsPath)) {
            $lockPid = trim((string) @file_get_contents($lockAbsPath));
            if ($lockPid !== '' && ctype_digit($lockPid) && function_exists('posix_kill') && @posix_kill((int) $lockPid, 0)) {
                $this->session->set_flashdata('error', 'Ja existe um deploy em execucao (PID ' . $lockPid . ').');
                return redirect(site_url('mapos/atualizacoes'));
            }
        }

        $logAbsPath = FCPATH . ltrim($deployConfig['log_file'], '/\\');
        $logDir = dirname($logAbsPath);
        if (! is_dir($logDir)) {
            @mkdir($logDir, 0775, true);
        }

        $escapedScript = escapeshellarg($scriptAbsPath);
    ...
    {
        return [
            'branch' => (string) config_item('deploy_branch'),
            'script_path' => (string) config_item('deploy_script_path'),
            'log_file' => (string) config_item('deploy_log_file'),
    ...
    private function getDeployStatus(array $deployConfig)
    {
        ...
        $status = [
            'branch' => $branch,
            'script_path' => $deployConfig['script_path'],
            'log_file' => $deployConfig['log_file'],
            'lock_file' => $deployConfig['lock_file'],
            'allow_web_trigger' => $deployConfig['allow_web_trigger'],
            'is_windows' => stripos(PHP_OS, 'WIN') === 0,
            'script_exists' => is_file(FCPATH . ltrim($deployConfig['script_path'], '/\\')),
            'lock_exists' => is_file($lockAbsPath),
            'running' => false,
            'lock_pid' => null,
            'git_current' => null,
            'git_remote' => null,
        ...
        if ($status['lock_exists']) {
            $lockPid = trim((string) @file_get_contents($lockAbsPath));
            if ($lockPid !== '' && ctype_digit($lockPid)) {
                $status['lock_pid'] = (int) $lockPid;
                if (function_exists('posix_kill')) {
                    $status['running'] = @posix_kill((int) $lockPid, 0);
                } else {
                    $status['running'] = true;
                }
            }
    ...
    private function safeShell($command)
    {
        if (stripos(PHP_OS, 'WIN') === 0 || ! function_exists('shell_exec')) {
            return null;
        }

        $fullCommand = 'cd ' . escapeshellarg(FCPATH) . ' && ' . $command . ' 2>&1';
